...
// --- TransactionItem Component (New) ---
// This component renders a single expense/income item
const TransactionItem = ({ item, type }) => {
  const isExpense = type === 'expense';
  const itemColorClass = isExpense
    ? 'text-red-300' // Lighter red for text on dark glass
    : 'text-green-300'; // Lighter green for text on dark glass
  ...
  const itemVariants = {
    ...
    hover: {
      ...
      borderColor: isExpense ? 'rgba(252,165,165,0.5)' : 'rgba(110,231,183,0.5)', // Brighter border on hover
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center text-right sm:text-left">
        ...
        {item.time && ( // Display time if available
          <p className="text-sm text-white/60 ml-4 hidden md:block">
...
// --- ExpensesTab Component (Redesigned) ---
const ExpensesTab = () => {
  ...
  // Framer Motion Variants for section containers
  const sectionVariants = {
    ...
    visible: {
      ...
      y: 0,
      transition: {
        delay: 0.3, // Delay after App.jsx animations
        staggerChildren: 0.1, // Stagger items inside
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      variants={{ visible: { transition: { staggerChildren: 0.2 } } }} // Staggering the two sections
    >
      {/* Expense Section */}
      <motion.div
      ...
      >
        ...
        <div className="flex-grow overflow-y-auto max-h-[400px] md:max-h-[500px] custom-scrollbar pr-2">
          <AnimatePresence mode="popLayout">
            {expenses.length > 0 ? (
              expenses.map((item) => (
                <TransactionItem key={item.id || item.time} item={item} type="expense" />
              ))
            ) : (
              <motion.div
              ...
              >
                ...
                <p className="text-sm mt-2">Time to spend wisely!</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
      ...
      >
        ...
        <div className="flex-grow overflow-y-auto max-h-[400px] md:max-h-[500px] custom-scrollbar pr-2">
          <AnimatePresence mode="popLayout">
            {incomes.length > 0 ? (
              incomes.map((item) => (
                <TransactionItem key={item.id || item.time} item={item} type="income" />
              ))
            ) : (
              <motion.div
                className="text-white/70 text-lg md:text-xl text-center p-8 flex flex-col items-center justify-center h-full"
              ...
              >
                ...
                <p className="text-sm mt-2">Let's get that money!</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
